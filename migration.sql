...
-- 2. Actualizar función process_checkout
...
-- 3. Actualizar función update_kiosk_order (Si existe, para que admita p_payment_method y p_notes)
-- Como el Kiosko crea la orden primero como borrador (draft/pending) al elegir al cliente,
-- y luego la actualiza, hay que modificar update_kiosk_order.
CREATE OR REPLACE FUNCTION update_kiosk_order(
    p_order_id UUID,
    p_items JSONB,
    p_total DECIMAL,
    p_status TEXT,
    p_notes TEXT DEFAULT NULL,
    p_payment_method TEXT DEFAULT 'tpv'
) RETURNS VOID AS $$
BEGIN
    UPDATE orders 
