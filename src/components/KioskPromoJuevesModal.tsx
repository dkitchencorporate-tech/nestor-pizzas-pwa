...
export default function KioskPromoJuevesModal({ products, onClose, onAdd }: PromoJuevesModalProps) {
  ...
  // Elegibilidad gestionada desde el panel admin (columna `jueves_promo_eligible`
  // en `products`), no una categoría fija en código — así entran también las
  // Pizzas Blancas y cualquier pizza futura que Néstor marque como elegible.
  const validPizzas = products.filter(p => p.jueves_promo_eligible === true && p.is_active !== false);

  const handleAddPizza = (pizza: any) => {
    setSelectedPizzas(prev => [...prev, { product: pizza, note: '', instanceId: nextInstanceId.current++ }]);
  };

  const handleRemovePizza = (instanceId: number) => {
    setSelectedPizzas(prev => prev.filter(sp => sp.instanceId !== instanceId));
  };

  const handleNoteChange = (instanceId: number, value: string) => {
    setSelectedPizzas(prev => prev.map(sp => sp.instanceId === instanceId ? { ...sp, note: value } : sp));
  };
  ...
  const sortedSelected = [...selectedPizzas].sort((a, b) => b.product.price - a.product.price);

  const totalPrice = (pairs * 11) + (remainder > 0 ? sortedSelected[sortedSelected.length - 1].product.price : 0);

  const handleAddToCart = () => {
    if (selectedPizzas.length === 0) return;

    sortedSelected.forEach((sp, index) => {
      const isPromo = index < pairs * 2;
      onAdd({
        productId: sp.product.id,
        name: `${sp.product.name} ${isPromo ? `(${t('promo_title_2')})` : ''}`,
        price: isPromo ? 5.50 : sp.product.price,
        quantity: 1,
        notes: sp.note.trim() || undefined
      });
    });
  ...
  return (
    <div className="fixed inset-0 z-[1100] bg-black/85 backdrop-blur-md flex items-start sm:items-center justify-center p-2 sm:p-4 pt-12 sm:pt-4 overflow-y-auto no-scrollbar">
      <div className="bg-zinc-900 border border-green-500/40 rounded-3xl sm:rounded-[2.5rem] w-full max-w-4xl overflow-hidden shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[94vh] animate-fade text-white relative">
                ...
                >
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-400 mt-1 line-clamp-2">
                      {lang === 'en' && pizza.description_en ? pizza.description_en : tDynamic(pizza.description)}
                    </p>
                  </div>
            ...
            <div className="p-4 sm:p-6 flex-1 overflow-y-auto no-scrollbar">
              ...
              {selectedPizzas.length === 0 ? (
                ...
                <div className="space-y-3">
                  {sortedSelected.map((sp, index) => {
                    const isPromo = index < pairs * 2;
                    return (
                      <div key={sp.instanceId} className="bg-zinc-900 border border-zinc-800 p-3 rounded-xl relative group">
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <span className="font-bold text-sm text-white block">{sp.product.name}</span>
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${isPromo ? 'text-green-400' : 'text-zinc-500'}`}>
                              {isPromo ? t('promo_price_label') : `${t('normal_price_label')} (${sp.product.price.toFixed(2)}€)`}
                            </span>
                          </div>
                          <button
                            onClick={() => handleRemovePizza(sp.instanceId)}
                            className="w-8 h-8 flex items-center justify-center bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-colors shrink-0"
                          >
                        ...
                        <input
                          ...
                          value={sp.note}
                          onChange={(e) => handleNoteChange(sp.instanceId, e.target.value)}
                          className="w-full mt-2 bg-zinc-950 border border-zinc-800 rounded-lg px-2.5 py-1.5 text-white text-xs focus:border-green-500 outline-none"
                        />
                      </div>
                    );
                  })}
        ...
        <div className="md:hidden p-3 bg-zinc-950/95 backdrop-blur-md border-t border-zinc-800 shrink-0 max-h-[45vh] overflow-y-auto no-scrollbar">
          {selectedPizzas.length > 0 && (
            <div className="space-y-2 mb-2">
              {sortedSelected.map((sp, index) => {
                const isPromo = index < pairs * 2;
                return (
                  <div key={sp.instanceId} className="bg-zinc-900 border border-zinc-700/70 rounded-xl p-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <span className="font-bold truncate text-[11px] text-white">{sp.product.name}</span>
                        <span className={`text-[9px] font-black shrink-0 ${isPromo ? 'text-green-400' : 'text-zinc-400'}`}>
                          {isPromo ? '5,50€' : `${sp.product.price.toFixed(2)}€`}
                        </span>
                      </div>
                      <button
                        onClick={() => handleRemovePizza(sp.instanceId)}
                        className="text-zinc-400 hover:text-red-400 font-bold text-xs shrink-0"
                      >
                        ✕
                      </button>
                    </div>
                    <input
                      ...
                      value={sp.note}
                      onChange={(e) => handleNoteChange(sp.instanceId, e.target.value)}
                      className="w-full mt-1.5 bg-zinc-950 border border-zinc-800 rounded-lg px-2 py-1 text-white text-[11px] focus:border-green-500 outline-none"
                    />
                  </div>
                );
              })}
