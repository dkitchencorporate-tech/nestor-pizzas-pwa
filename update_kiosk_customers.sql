...
-- PARCHE KIOSKO ERROR OVERLOADED
-- 1. Actualizar la versión SIN p_ip_address
CREATE OR REPLACE FUNCTION process_checkout(
    p_user_id UUID,
    p_client_name TEXT,
    p_client_phone TEXT,
...
BEGIN
    ...
    LOOP
        IF (v_item->>'product_id') IS NOT NULL THEN
            ...
            IF v_product_price IS NULL THEN RAISE EXCEPTION 'Producto no válido: %', (v_item->>'product_id'); END IF;
            v_frontend_price := (v_item->>'unit_price')::DECIMAL;
            IF v_frontend_price < v_product_price THEN
                IF v_is_thursday AND v_frontend_price = 5.50 AND v_product_category = 'NUESTRAS PIZZAS' AND (v_item->'customization_details'->>'name') ILIKE '%(Promo Jueves)%' THEN
                    v_jueves_promo_qty := v_jueves_promo_qty + (v_item->>'quantity')::INTEGER;
                ELSE
                    RAISE EXCEPTION 'Manipulación de precio detectada.';
                END IF;
            END IF;
        ELSE
            v_frontend_price := (v_item->>'unit_price')::DECIMAL;
        ...
        IF (p_points_redeemed) THEN
            IF (v_product_name ILIKE '%pizza%' OR v_product_name ILIKE '%burguer%') THEN
                IF (v_eligible_discount = 0 OR v_frontend_price < v_eligible_discount) THEN v_eligible_discount := v_frontend_price; END IF;
            END IF;
        END IF;
    END LOOP;
    
    IF v_jueves_promo_qty % 2 != 0 THEN RAISE EXCEPTION 'La promoción Jueves requiere pares.'; END IF;
    IF (p_points_redeemed AND v_actual_profile_id IS NOT NULL AND v_eligible_discount > 0) THEN
        IF EXISTS (SELECT 1 FROM profiles WHERE id = v_actual_profile_id AND points >= 25) THEN
            v_discount_applied := v_eligible_discount;
            UPDATE profiles SET points = points - 25 WHERE id = v_actual_profile_id;
    ...
    INSERT INTO orders (user_id, total_amount, status, client_name, client_phone, delivery_address, delivery_method, points_earned, points_redeemed, discount_applied) 
    VALUES (v_actual_profile_id, v_final_total, 'pending', p_client_name, p_client_phone, p_delivery_address, p_delivery_method, 0, CASE WHEN v_discount_applied > 0 THEN 25 ELSE 0 END, v_discount_applied) RETURNING id INTO v_order_id;
    
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
    LOOP
        INSERT INTO order_items (order_id, product_id, quantity, unit_price, customization_details) 
        VALUES (v_order_id, (v_item->>'product_id')::INTEGER, (v_item->>'quantity')::INTEGER, (v_item->>'unit_price')::DECIMAL, v_item->'customization_details');
    END LOOP;
...
-- 2. Actualizar la versión CON p_ip_address
CREATE OR REPLACE FUNCTION process_checkout(
    p_user_id UUID,
    p_client_name TEXT,
    p_client_phone TEXT,
...
BEGIN
    ...
    -- Bloqueo de IP (opcional si es necesario)
    IF p_ip_address != 'unknown' AND p_ip_address != 'kiosk' THEN
        -- lógica extra si aplicara
        NULL;
    END IF;
